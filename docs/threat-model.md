...
| `bcrypt(sha256(pin))` hashes + PBKDF2 salts / iteration counts | Raw PINs on new deliveries (`SHA-256(pin)` is the transport value) |
...
Summary matrix (details in §3.1–§3.17):

| # | Threat | Primary defense | Residual risk |
|---|---|---|---|
| 3.1 | Database compromise | Only ciphertext, wrapped keys, and hashes stored; keys require raw PINs | Offline brute-force of weak PINs where ciphertext survives |
| 3.2 | URL / token leakage | Links carry no key material; PIN required separately | URL + PIN leaking together over one channel |
| 3.3 | PIN leakage | Transport hashing; separate-channel distribution | A disclosed raw PIN defeats that copy until burn/revocation |
| 3.4 | Online PIN brute force | DB-backed throttle (`429`) + lockout self-destruct at 5 failures | Distributed sources reach the cap faster; total is still 5 |
| 3.5 | Parallel failed-PIN lost increments | Single-statement atomic `UPDATE … RETURNING` | None beyond §3.4 |
| 3.6 | Concurrent opens of a one-time secret | Advisory lock + row locks — exactly one winner | None within the model |
| 3.7 | View-count races | Counting inside the locked path | None identified |
| 3.8 | Revocation vs. in-flight access | Status re-checked inside the consumption lock | One possibly-in-flight response |
| 3.9 | Expiry vs. access | Expiry gate runs before any PIN work | Inter-instance clock skew window |
| 3.10 | Recipient compromise | Per-recipient isolation; surgical revocation | Already-decrypted/downloaded content |
| 3.11 | Creator compromise | `creator_token` gates management only — grants no decryption | Holder can recall/delete; inside trust boundary |
| 3.12 | Malicious served JavaScript | Static shipping + audit logging | Fully effective against any browser-crypto design (out of scope) |
| 3.13 | Direct Storage access | Private bucket, zero policies, service-role only | Service-role key possession (= server compromise) |
| 3.14 | Ciphertext tampering | AES-256-GCM authentication fails closed | Denial-of-service only |
| 3.15 | Dead-man's-switch failure | Destruction at next touch + daily purge sweep | Row exists unreadable until touch/sweep |
| 3.16 | File blobs lingering | Deletion wired into every lifecycle exit + purge backstop | Delayed deletion only, never extended availability |
| 3.17 | Downloaded file persistence | None — explicitly outside the model | Accepted; guidance in §3.17 |
...
- **Mitigation:** only ciphertext, wrapped keys, bcrypt hashes, and metadata exist there. Text/file keys require raw PINs (600k-iteration PBKDF2 per guess). New deliveries never expose raw PINs to the server at all. Lockout destruction and burn-after-read minimize how long ciphertext survives to be attacked offline.
...
- **Residual risk:** a 6-digit PIN space (10⁶ candidates) is brute-forceable offline given retained ciphertext (~6×10¹¹ SHA-256 ops per copy on typical hardware). Default burn-after-read, expiry, and lockout destruction exist to shrink that window; unconsumed long-lived deliveries remain the weakest-link case.
...
- **Mitigation:** database-backed sliding window (5 failed attempts per IP per delivery in 15 minutes → `429` + `Retry-After`, computed by the `check_pin_rate_limit` SQL function over audit events); independently, the 5th counted failure destroys that copy server-side — nothing remains to attack.
...
- **Evidence:** hardening suite: 12 simultaneous wrong PINs → counter reads 13 (every attempt counted, none lost), delivery locked and wiped, zero 5xx.
...
- **Evidence:** hardening suite: 20-way simultaneous valid-PIN open → exactly one `200`, losers get 409/410/429, replay → 410; file suite: 6-way race → exactly one winner; re-verified end-to-end through the real browser UI (10 simultaneous correct-PIN opens of a max-views=1 delivery → exactly 1 winner served ciphertext, 9 structured rejections, winner decrypted correctly client-side).
...
- **Evidence:** hardening suite: expired-delivery denial; deadman suite covers deadline-driven transitions.
...
- **Residual risk:** denial-of-service only — a tampered delivery becomes undecryptable.
...
- **Scenario:** encrypted file blobs linger after their delivery should be gone (burned, expired, revoked, locked out, deleted, or swept).
- **Mitigation:** deletion is wired into every lifecycle exit: burn-after-read removes the blob once no consumable recipient copy remains (`deliveryHasLiveCopies` check); whole-delivery revocation, creator deletion, dead-man's-switch destruction, and lazy expiry remove it unconditionally; the daily purge additionally expires stale actives (deleting blobs), wipes recipients of expired deliveries, deletes rows dead >30 days with a blob sweep, and prunes old events. Missing objects mid-access yield `410` plus a `destroyed` audit event rather than errors.
...
>
> Across seven suites, executed against the live Supabase project:
